
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta http-equiv="X-UA-Compatible" content="ie=edge">
  <title>Factura de Compra</title>
  <style>
  .attendance-table table{
  width: 100%;
  border-collapse: collapse;
  border: 1px solid #000;
}
.blank-cell{
  min-width: 50px;
}
.attendance-cell{
  padding: 8px;
}
.attendance-table table th.attendance-cell, .attendance-table table td.attendance-cell {
    border: 1px solid #000;
}
th {
    background-color:cornflowerblue;
    color: white;
}
h1 {
    background-color: #994432;
    color: white;
    border: 1px solid #000;
    position: center;
}
</style>
</head>
<body>
  <h1><center>EDG INFORMÁTICA</center></h1>
  <div>
    <img src="data:image/jpeg;base64," align="right">
  </div>
  <h2>Datos del Proveedor</h2>
  <div>
    <table>
      <tr>
        <th style="background-color:darkblue">Nombre</th>
        <th style="background-color:darkblue">Telefono</th>
        <th style="background-color:darkblue">Localidad</th>
        <th style="background-color:darkblue">Porvincia</th>
        <th style="background-color:darkblue">Pais</th>
      </tr>
      <tr>
        <th style="background-color:gray">{{$compra->proveedor->nombre}}</th>
        <th style="background-color:gray">{{$compra->proveedor->telefono}}</th>
        <th style="background-color:gray">{{$compra->proveedor->domicilio->location->name}}</th>
        <th style="background-color:gray">{{$compra->proveedor->domicilio->location->province->name}}</th>
        <th style="background-color:gray">{{$compra->proveedor->domicilio->location->province->state->name}}</th>
      </tr>
    </table>
  </div>
  <div>
      <h4>Domicilio:</h4>
      <p>
          <h4>Barrio: {{$compra->proveedor->domicilio->barrio}}  -     Calle: {{$compra->proveedor->domicilio->calle}}-Altura: {{$compra->proveedor->domicilio->numero}}</h4>
      </p>
  </div>
  <div class="attendance-table">
        <p><table class="table-bordered">
          <tr>
            <th class="attendance-cell"><strong>#</strong></th>
            <th class="attendance-cell">PRODUCTO</th>
            <th class="attendance-cell">SUBTOTAL</th>
            <th class="attendance-cell">PRECIO DE COSTO</th>
            <th class="attendance-cell">PRECIO DE VENTA</th>
            <th class="attendance-cell">CANTIDAD</th>
         
          </tr>
          
              @foreach($compra->lineaCompra as $l)
              <tr>
                    <td class="attendance-cell">{{$l->producto->id}}</td>
                    <td class="attendance-cell">{{$l->producto->descripcion}}</td>
                    <td class="attendance-cell">{{$l->subTotal}}</td>
                    <td class="attendance-cell">${{$l->producto->precio_venta}}</td>
                    <td class="attendance-cell">${{$l->producto->precio_costo}}</td>
                    <td class="attendance-cell">{{$l->cantidad}} Unidades</td>
              </tr>
              @endforeach
          
        </table></p>
        <p><h3>Importe Total: ${{$compra->monto}}</h3>
</body>
</html>